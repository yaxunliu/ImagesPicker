...
//    }
    ...
    func encrypt() -> String? {
        let phoneArr = self.map { character in
            return String(character)
        }
        if phoneArr.count != 11 { return nil }
        let sercuteCodes = ["!", "@", "#", "$", "%", "^", "&", "*", "(", ")", "_"]
        let result = zip(sercuteCodes, phoneArr)
        var strM = ""
        for i in result {
            strM = strM + i.0 + i.1
        }
    ...
    /// 匹配股票
    func matchStock(_ block: (_ range: NSRange, _ info: [String:String])->()) {
        do {
            let regex = try NSRegularExpression.init(pattern: "\\$[\\u4e00-\\u9fa5]{1,}\\([s,S][z,Z,h,H]\\d{6}\\)\\$", options: NSRegularExpression.Options.caseInsensitive)
            regex.enumerateMatches(in: self, options: .reportCompletion, range: NSMakeRange(0, self.count)) { (result, flag, stop) in
                guard let range = result?.range else { return }
                let str = NSString.init(string: self).substring(with: range)
                guard let stockIndex = str.index(of: "(") else { return }
                guard let stockEndIndex = str.index(of: ")") else { return }
                guard let nameIndex = str.index(of: "$") else { return }
                let nameStartIndex = str.index(nameIndex, offsetBy: 1)
                let stockStartIndex = str.index(stockIndex, offsetBy: 1)
                let stockName = str[nameStartIndex..<stockIndex]
                let stockSymbol = str[stockStartIndex..<stockEndIndex]
                block(range, ["name": String(stockName), "symbol": String(stockSymbol)] )
            }
        } catch let error {
            print("error", error)
        }
